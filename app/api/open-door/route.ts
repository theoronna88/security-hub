    ...
    try {
      ...
      if (response.status === 401) {
        ...
        if (authHeader && authHeader.includes('Digest')) {
          const digestResponse = await handleDigestAuth(url, username, password, authHeader);
          const responseText = await digestResponse.text();
          
          return NextResponse.json({
            success: digestResponse.ok,
            message: digestResponse.ok ? 'Porta aberta com sucesso' : 'Falha ao abrir porta',
      ...
      return NextResponse.json({
        success: response.ok,
        message: response.ok ? 'Porta aberta com sucesso' : 'Falha ao abrir porta',
        response: responseText,
...
async function makeDigestRequest(url: string, username: string, password: string) {
...
async function handleDigestAuth(url: string, username: string, password: string, authHeader: string) {
